...
**T19. Streamed textures could not be upscaled (chapter 10)**

**Symptom.** A stage that streams textures through reused memory addresses
(chapter 10 the clearest case) showed almost no upscaled surfaces, however
thoroughly it was dumped and processed.

**Cause.** The pack was matched at texture CREATION, on the bytes then in
memory; for a streamed texture those are a leftover from the previous
occupant of that address. The lookup keyed on a hash the pack never held and
fell back. The dump, by contrast, hashes at LOAD, on the final bytes, so the
pack was keyed correctly but consulted at the wrong moment. See also the
abandoned first attempt below.

**Fix.** In the shared GPU plugin (`texture_cache.cpp`): match at LOAD, on the
bytes really in memory (the same bytes and CRC the dump records). The guest
resource stays native-sized and always decoded, so nothing can go missing; a
texture gains a SEPARATE upscaled resource, which the sampler view prefers,
only when its content is found in the pack, rechecked every load so a reused
address stays correct. A content not in the pack simply samples the guest
texture at original resolution. Unmatched loads consult the in-memory index
and never touch the disk. Cvar `texture_pack_resolve_at_load`, default on.
Two dead ends preceded it: speculatively sizing the resource at 4x by id made
undumped streamed variants vanish (a 4x resource with no data), and there is
no safe fallback for that without either recreating the resource at load or a
scaling decode - which is why the separate-resource design was taken instead.
Verified in chapter 10: ~1500 textures upscaled where ~400 did before, at
full frame rate, nothing missing (user-confirmed 2026-09-12).
...
**S23. Dumping mid-stage silently missed most of the stage**

**Symptom.** Turning texture dumping on while in a stage captured very little,
so the pack never gained the stage's textures and it read as "dumping does
nothing".

**Cause.** Dumping only writes textures as they load. Enabled mid-stage, the
textures already resident and the whole start of the stage were never
re-loaded, so they were never written.

**Fix.** Dumping is now a restart-required setting (`src/ng2_menu.cpp`): it is
delivered only in the launch tuning file, never pushed live, and the menu
marks it restart-required with a note that it captures the stage from its
start on the next launch. So every texture the stage loads passes through the
dump path.
